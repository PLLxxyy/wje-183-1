...
export interface DeductionInput {
  ...
  socialInsuranceRate: number; // 社保比例 %
...
export interface TaxResult {
  ...
  housingFundMonthly: number;
  totalSocialFundMonthly: number;
  specialDeductionsMonthly: number;
  specialDeductionsDetail: { name: string; amount: number }[];
  ...
  netMonthly: number;
  netYearly: number;
  bracketDetails: { range: string; rate: number; taxableAmount: number; taxAmount: number }[];
}
...
export function calculateTax(
  ...
  // 专项附加扣除
  const specialDeductionsDetail: { name: string; amount: number }[] = [];
  ...
  const taxableIncomeMonthly = Math.max(
    0,
    grossMonthly - MONTHLY_THRESHOLD - totalSocialFundMonthly - specialDeductionsMonthly
  );
  const taxableIncomeYearly = taxableIncomeMonthly * 12;

  // 累进税计算
  const bracketDetails: TaxResult['bracketDetails'] = [];
...
function estimateTaxSaving(
  ...
  // 简化估算：用当前边际税率
  if (currentResult.taxableIncomeMonthly <= 0) return 0;

  let marginalRate = 0.03;
  const taxable = currentResult.taxableIncomeMonthly;
  if (taxable > 80000) marginalRate = 0.45;
  else if (taxable > 55000) marginalRate = 0.35;
  else if (taxable > 35000) marginalRate = 0.30;
  else if (taxable > 25000) marginalRate = 0.25;
  else if (taxable > 12000) marginalRate = 0.20;
