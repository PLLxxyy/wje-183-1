...
for (const tc of testCases) {
    try {
        ...
        strictEqual(result.applicableRate, tc.expectedRate, tc.name + ': 适用税率不匹配');
        strictEqual(result.taxAmount, tc.expectedTax, tc.name + ': 税额不匹配');
        strictEqual(result.netBonus, tc.expectedNet, tc.name + ': 税后奖金不匹配');
        const monthlyBonus = round2(tc.bonus / 12);
        strictEqual(result.monthlyBonus, monthlyBonus, tc.name + ': 月均奖金不匹配');
        ok(result.bracketRange.length > 0, tc.name + ': 税率区间不能为空');
        console.log('✓ ' + tc.name);
        passed++;
    }
    catch (e) {
